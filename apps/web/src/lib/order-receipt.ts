...
export function buildReceiptHtml(order: ReceiptOrder, locale = "vi"): string {
  ...
  const body = [
    ...
    !(order.earnedPoints) ? (
      ...
      `<img src="https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(`${env.SITE_URL}/loyalty?code=${order.paymentCode}`)}" width="80" height="80" style="display:block;margin:0 auto;" alt="QR"/>` +
