...
const STEP_LABELS = ["Nhập SĐT", "Đặt lại mật khẩu"];
  ...
  if (step === "success") {
    return (
      ...
      >
        ...
        <Link
          href={route("LOGIN")}
          className="flex h-11 w-full items-center justify-center rounded-xl bg-[#1a3c34] text-sm font-semibold text-white hover:bg-[#2d4a43] transition"
        >
          Về trang đăng nhập
        </Link>
      </motion.div>
        ...
        {step === "phone" ? (
          ...
          >
            <p className="text-sm leading-relaxed text-foreground/55">
              Nhập số điện thoại đã đăng ký. Chúng tôi sẽ gửi mã OTP để xác
              minh danh tính.
            </p>
          ...
          >
            ...
            {resetPassword.isError && (
              <div className="rounded-xl border border-red-100 bg-red-50 px-3.5 py-2.5 text-xs text-red-700">
                {axiosErrorMessage(resetPassword.error)}
              </div>
            )}

            <button
              type="submit"
              disabled={!step2Valid || resetPassword.isPending}
              className="flex h-11 w-full items-center justify-center rounded-xl bg-[#1a3c34] text-sm font-semibold text-white transition hover:bg-[#2d4a43] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <div className="flex items-center justify-between text-xs">
              <button
                type="button"
                onClick={() => { setStep("phone"); setOtp(""); sendOtp.reset(); }}
  ...
  return (
    <AuthSplitLayout>
      ...
      <div className="mb-8 space-y-1">
        ...
        <p className="text-sm text-foreground/50">
          Xác minh số điện thoại để đặt lại mật khẩu mới.
        </p>
      </div>
